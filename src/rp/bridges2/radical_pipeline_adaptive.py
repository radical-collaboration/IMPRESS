...
USE_GPU = True
MACHINE = 'bridges2'
BASE_PATH = f"/jet/home/{os.environ['USER']}/impress/IMPRESS/src/rp"
ProteinMPNN_PATH = f"/jet/home/{os.environ['USER']}/impress/ProteinMPNN"
# File to store all required flags to run AlphaFold
#FLAGFILE = f'{BASE_PATH}/MACHINE_NAME/reduced_db.ff'
PIPELINE_NAMES = ['p1', 'p2']
TASK_PRE_EXEC  = ['module load anaconda3',
                f"source activate base",
                f"conda activate /jet/home/{os.environ['USER']}/impress/ve.impress"]

if USE_GPU:
    RES_FILE = 'access.bridges2'
    NUM_GPU = 1
    GPUS_PER_PILOT = 4
...
class Pipeline:
    ...
    def submit_next(self):
        ...
        print(f'\n{self.name} calling submit_next# {next_stage_id}')
    ...
    def submit_s1(self):
        ...
        self.tmgr.submit_tasks(rp.TaskDescription({
            ...
            #'launcher_args': ' -cpu-bind=cores --gres=gpu:0'
        }))
        return 1
        ...
        for fastas in self.fasta_list_2:
            ...
            tds.append(rp.TaskDescription({
                ...
                'arguments': [f'{self.base_path}/make_af_fasta.py',
                              ...
                              f'--seq={seq_id[0]}'],
                'pre_exec': TASK_PRE_EXEC,
                #'launcher_args': ' --export=ALL --exact --nodes 1 --ntasks 1 --cpus-per-task 1 --mem 0 --cpu-bind=cores --gres=gpu:0'
            }))
        ...
        for fastas in self.fasta_list_2:
            ...
            #    'pre_exec': TASK_PRE_EXEC,
                'post_exec': ['cp '
                              ...
                              + f'{fastas_2}.pdb'],
                'gpus_per_rank': 1,
                #'launcher_args': ' --export=ALL --exact --nodes 1 --ntasks 1 --cpus-per-task 1 --mem 0 --gres=gpu:1 --cpu-bind=cores  --gres-flags=enforce-binding --gpu-bind=single:1'
            }))
            # tds.append(rp.TaskDescription({
            #     'uid': ru.generate_id(f'{self.name}.3.%(item_counter)06d',
            #                           ru.ID_CUSTOM, ns=self.tmgr.session.uid),
...
def set_up_new_pipeline_dirs(pipe_name):
    ...
    path = f"{BASE_PATH}/af_pipeline_outputs_multi/{pipe_name}/af/prediction/best_ptm/"
    if not os.path.exists(path):
        os.makedirs(path, exist_ok=True)

    path = f"{BASE_PATH}/af_pipeline_outputs_multi/{pipe_name}/af/prediction/dimer_models/"
    if not os.path.exists(path):
        os.makedirs(path, exist_ok=True)

    path = f"{BASE_PATH}/af_pipeline_outputs_multi/{pipe_name}/af/prediction/logs"
    if not os.path.exists(path):
        os.makedirs(path, exist_ok=True)
        
    for i in range(1,6): 
        path = f"{BASE_PATH}/af_pipeline_outputs_multi/{pipe_name}/mpnn/job_{i}"
        if not os.path.exists(path):
            os.makedirs(path, exist_ok=True)
