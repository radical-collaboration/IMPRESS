...
async def run_pipeline_async(input_pdb_filename: str, backend: DragonExecutionBackendV3, endpoint_cycle: itertools.cycle, verbose: bool = True):
    """
...
#            endpoint_cycle = state.get("endpoint_cycle")            
        ...
        initial_state["endpoint_cycle"] = endpoint_cycle
        ...
        if verbose:
            ...
            print(f"Router Mode: {'JSON Parsing'}")
            print("=" * 70)
...
async def main():
    ...
    # Run the pipeline
    final_state = await run_pipeline_async(input_pdb, backend, endpoint_cycle, verbose=True)
