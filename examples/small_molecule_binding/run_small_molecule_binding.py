...
async def adaptive_decision(pipeline: SmallMoleculeBindingPipeline) -> Optional[Dict[str, Any]]:
    ...
    # TODO Read current scores from a CSV file containing relevant scores
#    scores_file = f'name_of_file_{pipeline.name}_pass_{pipeline.passes}.csv'
#    with open(scores_file) as fd:
...
#            pipeline.current_scores[protein] = float(score_str)
    ...
    # FORNOW ...
    protein="3rk4"
    pipeline.current_scores[protein] = random.random()
