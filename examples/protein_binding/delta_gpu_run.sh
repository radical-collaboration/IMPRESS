...
# asyncflow writes its session dir (runinfo, captured task stdout/stderr as
# .stdout/.stderr per task UID) under the current working directory, which is
# ${WORKDIR} above — on Lustre, so the files survive the job and can be
# reviewed after a failure.
